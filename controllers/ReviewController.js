...
module.exports.create = function(req, res) {
	...
	Review.create(req.body, function(err, resp) {
		...
		Album.findOne({'albumSpotifyId': reviewObj.albumSpotifyId}, function(err, album) {
			if(err) return res.status(500).json(err);
			...
			if(!album) {
				Album.create({
					...
					reviews: [{review: resp._id, user: resp.userId}],
			...
			if(album) {
				album.reviews.push({review: resp._id, user: resp.userId});
...
module.exports.update = function(req, res) {
	...
	Review.findByIdAndUpdate(req.params.id, req.body, { new: true }, function(err, result) {
